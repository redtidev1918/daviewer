...
Windows 版解压即可运行：登录在 App 内完成，不需要管理员权限、不安装服务，也不读取
系统密码；移动安装目录后重新启动即可。

> **关于 macOS 版本**：这是**未经过 Apple 签名、未公证的测试版**。首次启动请在 Finder 中
> 右键点 App 图标并选择「打开」即可。
...
- **作者**：资料（含简介、关注者数、加入时间）、画廊（可搜索作者自己的作品）、Scraps、
  自定义分画廊（画集）、收藏夹、关注
...
macOS 预览包使用稳定的项目签名，登录数据保存在系统钥匙串的独立
`DAViewer Account` 项目中；待处理的登录记录只是崩溃恢复数据，无法保存或清理时
也不会阻断当前登录。
...
- **macOS 首次登录的钥匙串授权**：系统可能询问是否允许 App 访问钥匙串（例如
  「DAViewer Account 想要使用机密信息」），请选择「允许」或「始终允许」——这是
  App 在本机安全保存登录状态所需的正常授权，不会上传任何数据，也不会读取其他密码。
  如果系统因钥匙串锁定而要求输入 Mac 密码，那是 macOS 解锁本机钥匙串的正常提示；
  App 始终不会收集或上传你的密码。
