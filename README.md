...
数字作品 ID（网页版推荐流）需要先通过网页会话换成官方 API 使用的 UUID。
如果 CSRF 会话刚好轮换，「更多类似作品」会显示原生重试入口；重试会等待后台网页会话
真正刷新完成，再重新执行 ID 解析，不会静默隐藏整个区块。

首次安装会直接进入统一登录页。个性化推荐只在网页身份与 CSRF 会话都准备完成后
请求；未登录属于正常引导状态，不会显示成推荐加载错误。OAuth 失败会保留登录页并
显示原因与重试按钮。
...
- **macOS 的「钥匙串」提示**：首次登录时 macOS 可能弹窗「DAViewer 想要使用钥匙串中的机密信息」。这是 macOS 对**所有**把登录凭据存进「钥匙串」的应用的统一安全确认。DAViewer 只会在钥匙串里保存**你登录 DeviantArt 后拿到的 OAuth 令牌**，不读取、不保存其它任何机密（如浏览器密码、Wi-Fi 密码）。该项已命名为「DAViewer」并补齐了沙盒应用的钥匙串访问授权，正常登录后不会反复弹窗。
