...
- **发现流**：首页「发现」使用官方 OAuth `browse/home` 通用发现内容；不冒充网页版个性化推荐
...
首页是**原生界面**（发现 / 每日精选 两个标签），底部另有**「关注动态」一级标签**
（即 DeviantArt 的 `/watch/deviations`，关注画师的最新作品，顶部带按更新时间排序的
头像排）。「发现」是官方 `browse/home` 通用发现流，并不等于网页版 Cookie 驱动的
个性化 `rfy/deviations`；App 不再用错误名称承诺两者一致。「发现」和「每日精选」都使用
官方 OAuth API，与收藏、关注、下载共用同一份登录态。App 内嵌网页中的官方页负责账号、注册、社交提供商和安全验证；
`dakit://oauth/callback` 在网页内被拦截并完成登录，没有第二份网页身份需要同步。
