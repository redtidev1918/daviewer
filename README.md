...
- **作品详情**：图片统一双击/捏合缩放；视频优先最高画质、可拖进度并支持失败重试；GIF 带缓存和加载进度；多图作品左右翻页；富文本图片带缓存、占位和重试
- **相关内容**：详情页底部原生展示「更多类似作品」瀑布流；稀疏数据自动补全，失败或空结果均有可见状态和刷新入口
...
真正刷新完成，再重新执行 ID 解析。官方预览响应只带作品 ID 时，DAKit 会用受控并发
补全作品详情；真实空结果也会保留说明和重试入口，不再静默隐藏整个区块。

首次安装会直接进入统一登录页。个性化推荐只在网页身份与 CSRF 会话都准备完成后
请求；未登录属于正常引导状态，不会显示成推荐加载错误。首次登录严格先建立网页
Cookie/CSRF，再复用该会话完成 OAuth，避免两个页面抢导航。OAuth 失败会保留登录页并
显示原因与重试按钮。
