...
- 网页版功能齐全（个性化推荐、画廊、标签、收藏、关注、下载），但缺少桌面/移动原生体验；
- 本项目把网页版的完整能力与原生交互结合起来，开箱即用，无需自己注册 OAuth 应用。
...
- **作品详情**：左右滑动或顶部按钮切换前后作品；双指缩放；多图分页
- **媒体**：图片统一缩放；视频最高画质、可拖进度、失败重试；GIF 与富文本图片带缓存和加载进度
- **相关内容**：详情页原生展示「更多类似作品」，空结果与失败有明确提示
- **标签**：详情/搜索/标签页统一标签条，缺失标签自动从官方数据补全
- **作者**：资料（含简介）、画廊、自定义分画廊（画集）、收藏夹、关注
- **社交**：收藏（含收藏态）、关注/取关作者、关注列表、通知
- **下载**：实时确认原图权限；不可下载时说明登录/购买/次数/作者原因，可降级保存最高画质预览；支持打开文件/文件夹与删除确认
- **双语**：中文 / English 切换
...
应用运行时会自动读取系统代理（macOS 通过 `scutil`，Windows 通过注册表），也可在
「设置 → 网络代理」手动填写 `host:port`。App 自身也识别 `http_proxy` /
`https_proxy` / `all_proxy` 及其大写形式；从 Finder 直接启动时通常不继承终端
环境变量，此时请使用系统代理或应用内设置。
...
- **macOS 未签名测试版的「钥匙串」提示**：临时签名的应用身份可能随版本变化，首次登录或升级后，macOS 可能弹出「DAViewer 想要使用钥匙串中的机密信息」并要求输入 Mac 登录密码。该输入框属于 macOS 系统，DAViewer 不会读取密码；App 只访问自己保存的 DeviantArt OAuth 令牌。正式 Developer ID 签名与公证完成前，macOS 包会一直明确标记为未签名测试版。
