...
首次登录由同一个官方 OAuth/PKCE 导航同时建立网页 Cookie/CSRF 与 App OAuth，避免
社交账号登录后再启动第二轮授权；未登录是正常引导状态。详细边界
...
找回密码或先设置代理，然后才打开 DeviantArt 官方页面。首次登录从 OAuth 地址开始，
登录页和社交弹窗都回到同一个 PKCE 事务，一次完成网页会话与 App 授权。两条会话日后
不同步时，首页顶部会显示提示条，一键补全；WebView 不可用时自动回退系统浏览器。
...
- **Google / Apple 登录**：原生入口分别提供 Google 与 Apple 按钮，点击后直接触发 DeviantArt 官方页的对应提供商，并在原 OAuth 事务内接回结果；不会先网页登录再要求第二次授权。移动端使用桌面登录布局，并在 App 内承接官方弹窗与 Cookie。
- **登录前检查代理**：原生登录入口会显示当前网络路径，可直接进入代理设置或运行连通性测试，不必等网页白屏后再排查。
- **出现人机验证**：代理出口可能触发 DeviantArt 的安全检查。它不是断网；直接在 App 当前页面完成验证即可，登录会沿用原授权流程自动继续。
